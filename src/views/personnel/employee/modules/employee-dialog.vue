<template>
  <ElDialog
    ...
    :title="dialogType === 'add' ? '新员工入职' : dialogType === 'view' ? '查看详情' : '编辑员工'"
    width="600px"
    align-center
    @close="handleClose"
  >
...
<script setup lang="ts">
  ...
  import { createUser, updateUser, getDepartmentOptions } from '@/api/system-manage'
  ...
  // const roleList = ref<Api.SystemManage.RoleListItem[]>([]) // Role list not needed for display
  ...
  const rules = computed<FormRules>(() => ({
    ...
    // roleIds: [{ required: true, message: '请选择角色', trigger: 'change' }], // Validation removed
    hireDate:
  ...
  const loadBasicData = async () => {
    try {
      ...
      // Role options loading suppressed since UI is hidden
      /* 
  ...
  const initFormData = () => {
    ...
    // 提取角色ID列表 (Kept to preserve roles on edit)
    let roleIds: number[] = []
